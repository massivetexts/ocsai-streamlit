...
ocsai2_langs = ['ara', 'chi', 'dut', 'eng', 'fre', 'ger', 'heb', 'ita', 'pol', 'rus', 'spa']
langref = {
        "eng": "English",
        "spa": "Spanish",
        "fre": "French",
    ...
    if uploaded_file is not None:
        ...
        for i in range(n_chunks):
            ...
            if q_template is not None:
                chunk['question'] = chunk['prompt'].apply(lambda x: q_template.format(prompt=x))
                inputs = chunk[[name_of_prompt_column, 'question', name_of_response_column]]
            else:
        ...
        merged = df.merge(scored.drop_duplicates(['prompt', 'response']), on=['prompt', 'response'],
                          how='left').drop(columns=['confidence', 'flags', 'language', 'type'])
        return merged
...
with st.sidebar:
    ...
    if model in ocsai2_models:
        language = st.selectbox("language", [None, 'CUSTOM'] + ocsai2_langs,
                                help=("Select a language. "
                                      "Use `CUSTOM` to specify your own language, "
                                      "or `None` to leave it to the model to figure out."),
                                index=2 + ocsai2_langs.index(default_lang) if default_lang in ocsai2_langs else 0,
                                format_func=lambda x: langref[x] if x in langref else x)
        
        if language == 'CUSTOM':
            language = st.text_input("Custom 3-character language code "
                                    "([reference](https://www.loc.gov/standards/iso639-2/php/English_list.php))")

        task = st.selectbox("task", [None, 'CUSTOM'] + tasks,
                            help=("Select a task. "
        ...
        change_question_template = st.checkbox("Change question template",
                                               value=False,
                                               help=("If you want to specify a custom question "
        ...
        if change_question_template:
            ...
            q_template = st.text_input("Question template",
                                       value=default_template,
                                        
                                       help=("This is the question template that will be used to "
    ...
    upload_format = st.selectbox("Upload format",
                                 help=("If `auto`, the file extension is used to determine the "
                                       "format."),
                                 options=["csv", "excel", "auto"],
                                 index=2)
    name_of_prompt_column = 'prompt'
    name_of_response_column = 'response'
    custom_names = st.checkbox("Use custom column names",
                               value=False,
                               help=("The first row of your data should name your columns. "
